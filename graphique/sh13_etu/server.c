...
void sendOwnLine(int playerID, int playerport) {
	for(int i = 0; i < 8; i++) {
		char lineMessage[32];
		sprintf(lineMessage,"V %d",tableCartes[playerID][i]);
		sendMessageToClient(tcpClients[playerID].ipAddress,tcpClients[playerID].port,lineMessage);
	}
}
        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
                                ...
                                if (nbClients==4)
				{
					...
					sendMessageToClient(tcpClients[0].ipAddress,tcpClients[id].port,reply);
					...
					sendMessageToClient(tcpClients[1].ipAddress,tcpClients[id].port,reply);
					...
					sendMessageToClient(tcpClients[2].ipAddress,tcpClients[id].port,reply);
					...
					sendMessageToClient(tcpClients[3].ipAddress,tcpClients[id].port,reply);
